...
* Dictionary Handling
  ...
  useful should the user wish to add a word in real time.
* TODO Strokes
  ...
  The bits should be in steno order, but it is not clear how the bits
  should be placed in the 32-bit int (or which direction to store the
  bits). This will probably end up being whatever is most convenient
  for look ups / reading from a device. Since shift registers will
  very likely be used in order to save pins it makes sense to put the
  bits in the lower end of the 32-bit integer. Which way these bits
  are stored doesn't really matter because we will just use these
  lower bits as the keys to the dictionary. It may be convenient to
  have them reversed so that the lowest bit represents S- because then
  it's easy to shift them off, but if we need to check if a specific
  key is pressed we're probably just going to use bit masks
  anyway... So it doesn't really matter. I am leaning towords the
  lowest bit being the last key in steno order since that makes some
  sense from a debugging perspective to see which keys are pressed. On
  the other hand the other way the number of the bit refers to the
  number of the key in steno order, which is actually very elegant and
  appealing, so maybe that's best!
* Translation
